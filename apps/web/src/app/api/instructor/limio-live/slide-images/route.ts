...
import { liveSlideImageKey } from "@/lib/storage-keys";

export const runtime = "nodejs";

// PDF được rasterize thành ảnh ngay trên client (pdfjs-dist) rồi upload từng
// trang qua endpoint này — cùng convention với
// instructor/teaching-tools/whiteboards/pages.
...
export async function POST(req: Request) {
  ...
  const key = liveSlideImageKey(now, filename);
  const buf = Buffer.from(await file.arrayBuffer());
  await storageFor(key).put(key.key, buf, file.type);

  return NextResponse.json(
    { ok: true, url: `/api/limio-live-media/${filename}`, filename, sizeBytes: file.size },
    { status: 201 },
  );
}
