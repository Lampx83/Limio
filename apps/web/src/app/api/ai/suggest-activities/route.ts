...
/**
 * Suggest generative learning activities for a piece of content. Body shape:
 *   { lessonId } — pull lesson title + content items as input
 *   { rawText } — fallback ad-hoc text
 *
 * Authz: instructor of the course or admin.
 */
export async function POST(req: Request) {
  ...
  const body = (await readJson(req)) as
    | { lessonId?: string; rawText?: string }
    | null;
  if (!body)
  ...
  try {
    const suggestions = await suggestActivitiesForContent(
      userId,
      contentText,
      openai,
    );
